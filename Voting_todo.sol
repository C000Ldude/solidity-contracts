...
function vote(uint256 proposalId, bool voteType) proposalExist(proposalId) excludeOwner(proposalId) notVoted(proposalId) active(proposalId) public{
...
function getVoters(uint256 proposalId)public view proposalExist(proposalId) returns(uint256){
    return Proposals[proposalId].upVotes + Proposals[proposalId].downVotes;
